...
import (
	...
	"github.com/crescent-network/crescent/v5/testutil/network"
)
...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(sdkserver.FlagMinGasPrices, fmt.Sprintf("0.000006%s", BaseDenom), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01photino,0.001stake)")
...
Example:
	crescentd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "crescentd", "Home directory of the node's daemon configuration")
...
Example:
	crescentd testnet start --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		kb, err := keyring.New(sdk.KeyringServiceName(), args.keyringBackend, nodeDir, inBuf, ethkeyring.Option())
		if err != nil {
			return err
		}
		...
		coins := sdk.Coins{
			sdk.NewCoin(BaseDenom, accStakingTokens),
		}
		...
		customAppTemplate, customAppConfig := config.AppConfig(BaseDenom)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, BaseDenom, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
