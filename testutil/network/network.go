...
func DefaultConfig() Config {
	...
	return Config{
		...
		ChainID:           fmt.Sprintf("crescent-1"),
		NumValidators:     4,
		BondDenom:         ethermint.AttoPhoton,
		MinGasPrices:      fmt.Sprintf("0.000006%s", ethermint.AttoPhoton),
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000, ethermint.PowerReduction),
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "crescentd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "crescentcli")
		...
		customAppTemplate, _ := config.AppConfig(cfg.BondDenom)
