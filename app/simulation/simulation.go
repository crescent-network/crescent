...
import (
	...
	simultaion "github.com/cosmos/cosmos-sdk/x/simulation"
)
...
func SimulateFromSeed(
	...
	randAccFn simulationtypes.RandomAccountFn,
	...
	fmt.Fprintf(w, "Randomized simulationtypes params: \n%s\n", mustMarshalJSONIndent(params))

	timeDiff := maxTimePerBlock - minTimePerBlock
	accs := randAccFn(r, params.NumKeys())
	eventStats := simultaion.NewEventStats()
	...
	var timeOperationQueue []simulationtypes.FutureOperation

	logWriter := simultaion.NewLogWriter(testingMode)
	...
	if !testingMode {
		...
		defer func() {
			if r := recover(); r != nil {
				_, _ = fmt.Fprintf(w, "simulationtypes halted due to panic on block %d\n", header.Height)
				logWriter.PrintLogs()
				panic(r)
			}
		}()
	...
	for height := config.InitialBlockHeight; height < config.NumBlocks+config.InitialBlockHeight && !stopEarly; height++ {
		...
		logWriter.AddEntry(simultaion.BeginBlockEntry(int64(height)))
		...
		logWriter.AddEntry(simultaion.EndBlockEntry(int64(height)))
	...
	if stopEarly {
		if config.ExportStatsPath != "" {
			fmt.Println("Exporting simulationtypes statistics...")
...
// initialize the chain for the simulationtypes
func initChain(
	r *rand.Rand,
	params Params,
	...
	config simulationtypes.Config,
	cdc codec.JSONCodec,
) (mockValidators, time.Time, []simulationtypes.Account, string) {
...
type blockSimFn func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context,
	accounts []simulationtypes.Account, header tmproto.Header) (opCount int)
...
func createBlockSimulator(testingMode bool, tb testing.TB, w io.Writer, params Params,
	...
	operationQueue OperationQueue, timeOperationQueue []simulationtypes.FutureOperation,
	logWriter simultaion.LogWriter, config simulationtypes.Config,
) blockSimFn {
	...
	return func(
		r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accounts []simulationtypes.Account, header tmproto.Header,
	) (opCount int) {
		_, _ = fmt.Fprintf(
		...
		for i := 0; i < blocksize; i++ {
			opAndRz = append(opAndRz, opAndR{
				op:   selectOp(r),
				rand: simulationtypes.DeriveRand(r),
			})
		}
		...
		for i := 0; i < blocksize; i++ {
			...
			if !config.Lean || opMsg.OK {
				logWriter.AddEntry(simultaion.MsgEntry(header.Height, int64(i), opMsg))
			}
...
// nolint: errcheck
func runQueuedOperations(queueOps map[int][]simulationtypes.Operation,
	height int, tb testing.TB, r *rand.Rand, app *baseapp.BaseApp,
	ctx sdk.Context, accounts []simulationtypes.Account, logWriter simultaion.LogWriter,
	...
	for i := 0; i < numOpsRan; i++ {
		...
		if !lean || opMsg.OK {
			logWriter.AddEntry((simultaion.QueuedMsgEntry(int64(height), opMsg)))
		}
...
func runQueuedTimeOperations(queueOps []simulationtypes.FutureOperation,
	height int, currentTime time.Time, tb testing.TB, r *rand.Rand,
	app *baseapp.BaseApp, ctx sdk.Context, accounts []simulationtypes.Account,
	logWriter simultaion.LogWriter, event func(route, op, evResult string),
