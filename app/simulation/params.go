...
func (p Params) LivenessTransitionMatrix() TransitionMatrix {
	return p.livenessTransitionMatrix
}

func (p Params) BlockSizeTransitionMatrix() TransitionMatrix {
...
func RandomParams(r *rand.Rand) Params {
	return Params{
		...
		numKeys:                   simulationtypes.RandIntBetween(r, 2, 2500), // number of accounts created for the simulation
		evidenceFraction:          r.Float64(),
		initialLivenessWeightings: []int{simulationtypes.RandIntBetween(r, 1, 80), r.Intn(10), r.Intn(10)},
...
type ParamChange struct {
	subspace string
	key      string
	simValue simulationtypes.SimValFn
}
...
func (spc ParamChange) SimValue() simulationtypes.SimValFn {
	return spc.simValue
}

// NewSimParamChange creates a new ParamChange instance
func NewSimParamChange(subspace, key string, simVal simulationtypes.SimValFn) ParamChange {
	return ParamChange{
		subspace: subspace,
		key:      key,
		simValue: simVal,
...
type WeightedProposalContent struct {
	...
	contentSimulatorFn simulationtypes.ContentSimulatorFn // content simulator function
}

func NewWeightedProposalContent(appParamsKey string, defaultWeight int, contentSimulatorFn simulationtypes.ContentSimulatorFn) simulationtypes.WeightedProposalContent {
...
func (w WeightedProposalContent) ContentSimulatorFn() simulationtypes.ContentSimulatorFn {
...
func randomConsensusParams(r *rand.Rand, appState json.RawMessage, cdc codec.JSONCodec) *abci.ConsensusParams {
	...
	consensusParams := &abci.ConsensusParams{
		Block: &abci.BlockParams{
			MaxBytes: int64(simulationtypes.RandIntBetween(r, 20000000, 30000000)),
			MaxGas:   -1,
		},
		...
		Evidence: &tmproto.EvidenceParams{
			MaxAgeNumBlocks: int64(stakingGenesisState.Params.UnbondingTime / simulation.AverageBlockTime),
