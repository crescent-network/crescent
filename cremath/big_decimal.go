...
var (
	ZeroBigDec  = BigDec{new(big.Int)}
	OneBigDec   = BigDec{new(big.Int).Set(precisionReuse)}
	SmallestDec = BigDec{new(big.Int).Set(oneBigInt)}
)
...
const (
	// Precision is the number of decimal places of BigDec
	Precision = 36

	// DecimalPrecisionBits is bits required to represent the above precision
	...
	maxDecBitLen = 256 + decimalTruncateBits // XXX
)
...
func (d BigDec) MulTruncateMut(d2 BigDec) BigDec {
	d.i.Mul(d.i, d2.i)
	d.i = chopPrecisionAndTruncate(d.i)
	if d.i.BitLen() > maxDecBitLen {
		panic("Int overflow")
	}
	return d
...
func chopPrecisionAndRoundUp(d, precision *big.Int) *big.Int {
	// remove the negative and add it back when returning
	if d.Sign() == -1 {
		// make d positive, compute chopped value, and then un-mutate d
		d = d.Neg(d)
	...
	// get the truncated quotient and remainder
	quo, rem := d, big.NewInt(0)
	quo, rem = quo.QuoRem(d, precision, rem)
...
func chopPrecisionAndRoundUpDec(d *big.Int) *big.Int {
	return chopPrecisionAndRoundUp(d, sdkPrecisionReuse)
}
...
func (d BigDec) Ceil() BigDec {
	...
	return NewBigDecFromBigInt(quo.Add(quo, oneBigInt))
}
