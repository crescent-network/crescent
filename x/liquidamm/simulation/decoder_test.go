...
func TestDecodeStore(t *testing.T) {
	...
	publicPosition := types.PublicPosition{}
	rewardsAuction := types.RewardsAuction{}
	bid := types.Bid{}

	kvPairs := kv.Pairs{
		Pairs: []kv.Pair{
			{Key: types.PublicPositionKeyPrefix, Value: cdc.MustMarshal(&publicPosition)},
	...
	tests := []struct {
		...
		{"PublicPosition", fmt.Sprintf("%v\n%v", publicPosition, publicPosition)},
