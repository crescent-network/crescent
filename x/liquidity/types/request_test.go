...
func TestCancelOrderRequest_Validate(t *testing.T) {
	for _, tc := range []struct {
		name        string
		malleate    func(req *types.CancelOrderRequest)
		expectedErr string
	}{
		{
			"happy case",
			func(req *types.CancelOrderRequest) {},
			"",
		},
		{
			"zero id",
		...
		t.Run(tc.name, func(t *testing.T) {
			pair := types.NewPair(1, "denom1", "denom2")
			orderer := sdk.AccAddress(crypto.AddressHash([]byte("orderer")))
			msg := types.NewMsgCancelOrderBatch(orderer, pair.Id, 1)
			req := types.NewCancelOrderRequest(msg, 1, pair, 1)
			tc.malleate(&req)
			err := req.Validate()
			if tc.expectedErr == "" {
				require.NoError(t, err)
