...
func (s *KeeperTestSuite) TestExecuteDepositRequestWithDisabledPool() {
	...
	// Create a pool.
	pool := s.createPool(s.addr(0), parseCoin("1000000denom1"), parseCoin("1000000denom2"))

	// Send the pool's balance to an attacker's address.
	// In normal situations this wouldn't happen but there's still such threat.
	err := s.app.BankKeeper.SendCoins(ctx, pool.GetReserveAddress(), s.addr(2), s.getBalances(pool.GetReserveAddress()))
	...
	req := s.depositBatch(depositor, pool.Id, depositXCoin, depositYCoin)
