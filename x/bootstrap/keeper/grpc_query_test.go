...
func (suite *KeeperTestSuite) TestGRPCBootstraps() {
	ctx := suite.ctx
	k := suite.keeper
	...
	// apply market maker
	err := k.ApplyBootstrap(ctx, mmAddr, []uint64{1, 2, 3, 4, 5, 6})
	suite.NoError(err)
	err = k.ApplyBootstrap(ctx, mmAddr2, []uint64{2, 3, 4, 5, 6, 7})
	suite.NoError(err)

	// include market maker
	proposal := types.NewBootstrapProposal("title", "description",
	...
	for _, tc := range []struct {
		...
		expectErr bool
		postRun   func(response *types.QueryBootstrapsResponse)
	}{
		{
			"nil request",
			nil,
		...
		{
			"query all",
			&types.QueryBootstrapsRequest{},
			false,
			func(resp *types.QueryBootstrapsResponse) {
				suite.Require().Len(resp.Marketmakers, 12)
			},
		},
		{
			"query all with page limit",
			&types.QueryBootstrapsRequest{Pagination: &query.PageRequest{
				Limit: 10,
			}},
			false,
			func(resp *types.QueryBootstrapsResponse) {
		...
		},
		{
			...
			&types.QueryBootstrapsRequest{Eligible: "true"},
			false,
			func(resp *types.QueryBootstrapsResponse) {
				suite.Require().Len(resp.Marketmakers, 2)
		...
		},
		{
			...
			&types.QueryBootstrapsRequest{Eligible: "false"},
			false,
			func(resp *types.QueryBootstrapsResponse) {
				suite.Require().Len(resp.Marketmakers, 10)
				for _, mm := range resp.Marketmakers {
		...
		{
			"query with pair id",
			&types.QueryBootstrapsRequest{PairId: 3},
			false,
			func(resp *types.QueryBootstrapsResponse) {
				suite.Require().Len(resp.Marketmakers, 2)
				for _, mm := range resp.Marketmakers {
		...
		{
			"query with addr",
			&types.QueryBootstrapsRequest{Address: mmAddr.String()},
			false,
			func(resp *types.QueryBootstrapsResponse) {
		...
		},
		{
			...
			&types.QueryBootstrapsRequest{Address: mmAddr.String(), PairId: 1},
			false,
			func(resp *types.QueryBootstrapsResponse) {
		...
		},
		{
			...
			&types.QueryBootstrapsRequest{Address: "invalidaddr"},
			true,
			func(resp *types.QueryBootstrapsResponse) {
		...
		},
		{
			...
			&types.QueryBootstrapsRequest{Eligible: "invalidbool"},
			true,
			func(resp *types.QueryBootstrapsResponse) {
				suite.Require().Len(resp.Marketmakers, 0)
			},
		...
		suite.Run(tc.name, func() {
			resp, err := suite.querier.Bootstraps(sdk.WrapSDKContext(suite.ctx), tc.req)
			if tc.expectErr {
