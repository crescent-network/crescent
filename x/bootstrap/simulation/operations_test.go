...
func TestWeightedOperations(t *testing.T) {
	...
	weightedOps := simulation.WeightedOperations(appParams, cdc, app.AccountKeeper, app.BankKeeper, app.BootstrapKeeper)
	...
	expected := []struct {
		...
		{params.DefaultWeightMsgApplyBootstrap, types.ModuleName, types.TypeMsgApplyBootstrap},
...
// TestSimulateMsgApplyBootstrap tests the normal scenario of a valid message of type TypeMsgApplyBootstrap.
// Abnormal scenarios, where the message are created by an errors are not tested here.
func TestSimulateMsgApplyBootstrap(t *testing.T) {
	app, ctx := createTestApp(false)
	...
	params := app.BootstrapKeeper.GetParams(ctx)
	params.IncentivePairs = simulation.GenIncentivePairs(r)
	app.BootstrapKeeper.SetParams(ctx, params)

	// begin a new block
	app.BeginBlock(abci.RequestBeginBlock{Header: tmproto.Header{Height: app.LastBlockHeight() + 1, AppHash: app.LastCommitID().Hash}})

	// execute operation
	op := simulation.SimulateMsgApplyBootstrap(app.AccountKeeper, app.BankKeeper, app.BootstrapKeeper)
	operationMsg, futureOperations, err := op(r, app.BaseApp, ctx, accounts, "")
	require.NoError(t, err)

	var msg types.MsgApplyBootstrap
	err = types.ModuleCdc.UnmarshalJSON(operationMsg.Msg, &msg)
	require.NoError(t, err)

	require.True(t, operationMsg.OK)
	require.Equal(t, types.TypeMsgApplyBootstrap, msg.Type())
...
func TestSimulateMsgClaimIncentives(t *testing.T) {
	...
	err := app.BootstrapKeeper.DistributeBootstrapIncentives(ctx, []types.IncentiveDistribution{
		{
			Address: accounts[0].Address.String(),
			PairId:  1,
	...
	// execute operation
	op := simulation.SimulateMsgClaimIncentives(app.AccountKeeper, app.BankKeeper, app.BootstrapKeeper)
	operationMsg, futureOperations, err := op(r, app.BaseApp, ctx, accounts, "")
	require.NoError(t, err)
