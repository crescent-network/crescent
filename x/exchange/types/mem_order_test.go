...
func newUserMemOrder(
	orderId uint64, isBuy bool, price sdk.Dec, qty, openQty sdk.Int) *types.MemOrder {
...
func newOrderSourceMemOrder(
	isBuy bool, price sdk.Dec, qty sdk.Int, source types.OrderSource) *types.MemOrder {
	return types.NewOrderSourceMemOrder(
		utils.TestAddress(1), isBuy, price, qty, qty, types.DepositAmount(isBuy, price, qty), source)
}
...
func TestMemOrderBookSide_AddOrder(t *testing.T) {
	...
	require.Panics(t, func() {
		obs.AddOrder(newUserMemOrder(
			1, false, utils.ParseDec("12.3"), sdk.NewInt(100_000000), sdk.NewInt(80_000000)))
	})
	...
	for i := 0; i < 100; i++ {
		...
		obs.AddOrder(newUserMemOrder(
			...
			sdk.NewInt(100_000000), sdk.NewInt(90_000000)))
	}
	...
	require.Panics(t, func() {
		obs.AddOrder(newUserMemOrder(
			1, true, utils.ParseDec("12.3"), sdk.NewInt(100_000000), sdk.NewInt(80_000000)))
	})
	for i := 0; i < 100; i++ {
		tick := 9000 + r.Int31n(30)
		obs.AddOrder(newUserMemOrder(
...
func TestMemOrder_HasPriorityOver(t *testing.T) {
	...
	order1 := newUserMemOrder(1, false, price, sdk.NewInt(100_000000), sdk.NewInt(90_000000))
	order2 := newUserMemOrder(2, false, price, sdk.NewInt(110_000000), sdk.NewInt(80_000000))
	// order2 quantity > order1 quantity
	require.True(t, order2.HasPriorityOver(order1))

	order1 = newUserMemOrder(1, false, price, sdk.NewInt(100_000000), sdk.NewInt(80_000000))
	order2 = newUserMemOrder(2, false, price, sdk.NewInt(100_000000), sdk.NewInt(90_000000))
		...
		for j := 0; j < 100; j++ {
			...
			if r.Float64() <= 0.3 { // 30% chance
				...
				orders = append(orders, types.NewOrderSourceMemOrder(
					orderSourceOrdererAddr, true, price, qty, qty, types.DepositAmount(true, price, qty), source))
			} else {
