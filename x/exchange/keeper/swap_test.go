...
func (s *KeeperTestSuite) TestSwapExactIn() {
	...
	pool1 := s.CreatePool(creatorAddr, "ucre", "uusd", 100, utils.ParseDec("5"), true)
	s.AddLiquidity(
	...
	pool2 := s.CreatePool(creatorAddr, "uatom", "ucre", 100, utils.ParseDec("2"), true)
	s.AddLiquidity(
	...
	routes := []string{market1.Id, market2.Id}
