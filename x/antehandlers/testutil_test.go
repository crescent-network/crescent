...
func (suite *AnteTestSuite) SetupTest(isCheckTx bool) {
	...
	anteHandler, err := app.NewAnteHandler(
		app.HandlerOptions{
			...
			//BootstrapKeeper: &app.BootstrapKeeper,
		},
	)
...
// CreateTestTx is a helper function to create a tx given multiple inputs.
func (suite *AnteTestSuite) CreateTestTxWithMsgs(privs []cryptotypes.PrivKey, accNums []uint64, accSeqs []uint64, msgs []sdk.Msg, chainID string) (xauthsigning.Tx, error) {
	// First round: we gather all the signer infos. We use the "set empty
	// signature" hack to do that.
