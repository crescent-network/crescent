...
func (s *KeeperTestSuite) TestPoolOrdersMatching_Edgecase() {
	...
	for i := 0; i < 1; i++ {
		...
		for i := 0; i < 10; i++ {
			...
			_, res := s.PlaceMarketOrder(market.Id, ordererAddr, isBuy, qty)
			fmt.Println(res.ExecutedQuantity, res.LastPrice)
		}
	}
}
