...
func (s *KeeperTestSuite) TestQueryBestSwapExactAmountInRoutes() {
	...
	s.AssertEqual(utils.ParseCoin("972699556uusd"), resp.Output)
	...
	s.AssertEqual(utils.ParseCoin("95135827uatom"), resp.Results[0].Output)
	s.AssertEqual(utils.ParseCoin("142918uatom"), resp.Results[0].Fee)
	s.Require().EqualValues(3, resp.Results[1].MarketId)
	s.AssertEqual(utils.ParseCoin("95135827uatom"), resp.Results[1].Input)
	s.AssertEqual(utils.ParseCoin("972699556uusd"), resp.Results[1].Output)
	s.AssertEqual(utils.ParseCoin("1461241uusd"), resp.Results[1].Fee)
}
