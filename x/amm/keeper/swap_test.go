...
func (s *KeeperTestSuite) TestQueryBestSwapExactAmountInRoutes() {
	...
	s.AssertEqual(utils.ParseCoin("95135826uatom"), resp.Results[0].Output)
	s.AssertEqual(utils.ParseCoin("142919uatom"), resp.Results[0].Fee)
	s.Require().EqualValues(3, resp.Results[1].MarketId)
	s.AssertEqual(utils.ParseCoin("95135826uatom"), resp.Results[1].Input)
	s.AssertEqual(utils.ParseCoin("972699546uusd"), resp.Results[1].Output)
	s.AssertEqual(utils.ParseCoin("1461242uusd"), resp.Results[1].Fee)
}
